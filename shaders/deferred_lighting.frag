...
void main() {
    ...
    float diffuse_luminance = dot(normal, -lit_dir_direction);
    vec3 reflection = normalize(lit_dir_direction + 2.0 * normal);
    vec3 camera_ray = normalize(position - cam_position);
    float specular_intensity = max(0, pow(dot(-reflection, camera_ray), 16.0));
    vec3 specular_color = vec3(specular_intensity, specular_intensity, specular_intensity);

    color = vec4(lit_dir_color * diffuse_luminance + specular_intensity, 1.0);
}
